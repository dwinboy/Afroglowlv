...
import {
  ...
  ImageIcon, Settings,
  Scissors, Star, LogOut, Menu, X, ChevronRight,
  Home, Bell,
} from 'lucide-react'
...
const SIDEBAR_NAV = [
  ...
  { href: '/dashboard/portfolio', icon: ImageIcon,       label: 'Portfolio' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading && (!isAuthenticated || (user?.role !== 'BARBER' && user?.role !== 'ADMIN'))) {
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header className="h-16 bg-luxury-charcoal/80 backdrop-blur-md border-b border-luxury-border
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-xl text-gray-400 hover:text-white hover:bg-luxury-surface"
          >
            <Menu size={20} />
          </button>
        ...
        <div className="p-4 border-b border-luxury-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-gold flex items-center justify-center
                            text-luxury-black font-bold flex-shrink-0">
              {getInitials(user.fullName)}
            </div>
